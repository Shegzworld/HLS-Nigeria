document.addEventListener("DOMContentLoaded", function () {
	...
	const benfekServiceIconText = document.querySelectorAll(
		".benfek-services span"
	);
	...
	const nutrientGalleryContainer = document.querySelectorAll(
		".nutrient_gallery_container"
	);
	const nutrient_type_glass_pack = document.querySelector(
		".nutrient_type_glass_pack"
	);
	const nutrient_type_container = document.querySelector(
		"#nutrient_type_container"
	);
	...
	const NutrientTypeGlassCover = document.querySelectorAll(
		".Nutrient_type_glass"
	);
	const NutrientTypeHighlightButtons = document.querySelectorAll(".label-bg");
	const selected_nutrient_screen = document.querySelector(
		".selected_nutrient_screen"
	);
	...
	const researchSummaryItems = document.querySelectorAll(
		".research_summary_intro"
	);
	...
	// Codes for declaring Actives and event listeners for services to display 

	closeIcon.addEventListener('click', (e) => {
	console.log('yeah')
	e.stopPropagation(); 
	menuDropdown.style.transform = 'translateX(-100%)';
	menuDropdown.style.transition = 'transform 0.5s ease-in-out';
	});
	
	menuIcon.addEventListener('click', () => {
	menuDropdown.style.top = menuIcon.offsetTop + menuIcon.offsetHeight + 'px';
		...
		if (target.closest("#Its-a-News")) {
			...
			searchIcon.addEventListener("click", () => {
				if (isDisplayed) {
					...
					topStickyBar.style.height = "185px";
					isDisplayed = true;
				}
			});
			benfekServiceIconText.forEach((text) => {
	...
	const prevBtnNutrient_type = document.querySelector(
		".Nutrient_type_pack .slide_right"
	);
	let currentNutrientPictureView = 0;

	const updateNutrient_typeVisibility = () => {
		my_nutrient_picture.forEach((nutrientPic, index) => {
			nutrientPic.style.left = `${
				(index - currentNutrientPictureView) * 180
			}px`;
		});
	};
	...
	NutrientTypeGlassCover.forEach((glassCover) => {
		...
		console.log("clicked");
		supplementInfoButton =
			glassCover.parentElement.querySelectorAll(".view_info_btn");
	...
	researchSummaryItems.forEach((researchSummaryItem) => {
		researchSummaryItem.addEventListener("click", () => {
			...
			const isExpanded =
				extraContent.style.height !== "0px" && extraContent.style.height !== "";
			...
			if (isExpanded) {
				...
				extraContent.style.borderBottom =
					"solid rgba(100, 98, 98, 0.587) 0.01px";
	...
	viewInfoBtns.forEach((btn) => {
		btn.addEventListener("mouseenter", () => {
			...
			const shoppingCart =
				parentNutrientPicture.querySelector(".fa-shopping-cart");
