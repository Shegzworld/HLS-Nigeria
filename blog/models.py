...
class Blog(models.Model):
    ...
    slug = models.SlugField(unique=True, blank=True,
                            max_length=255)  # Make sure the slug is unique and can be empty initially
    ...
    try:
        ...
        if self.image_1 and hasattr(self.image_1, 'name'):
            try:
                ...
                logger.error(f"Error processing image_1: {e}")
                raise ValidationError(f"Invalid image_1: {e}")
        ...
        if self.image_2 and hasattr(self.image_2, 'name'):
            try:
                ...
                logger.error(f"Error processing image_2: {e}")
                raise ValidationError(f"Invalid image_2: {e}")
